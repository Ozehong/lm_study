...
1. 首先，将 𝑄 矩阵切为 𝑇𝑟 块（block），每块的长度为 𝐵𝑟 。用 𝑄𝑖 来表示切完后的某块矩阵，则 𝑄𝑖 的维度为 （𝐵𝑟，𝑑） 。不难理解， 𝑄𝑖 中存储着某 𝐵𝑟 个 token 的 query 信息
...
下面给出具体的图例加深理解（假设𝑇𝑟 等于 2， 𝑇𝑐 也等于 2）:
...
> 1. 我们假设标准场景下， $$S$$矩阵某一行的向量为 $$x = [x_1,x_2,\ldots,x_d]$$，因为分块的原因，它被我们切成了两部分 $$x = [x^{(1)},x^{(2)}]$$&#x20;
>
> 2. 我们定义：
>
>    * $$m(x)$$：标准场景下，该行的全局最大值
...
> ### **计算量**
>
> 1. 在算法第9行，我们有 $$S_{ij}=Q_iK_j^T$$ ，其中 $$Q_i\in\mathbb{R}^{B_r * d},K_j^T\in\mathbb{R}^{d * B_c}$$ 。根据前置知识，求 $$S_{ij}$$ 的计算量为 $$O(B_rB_cd)$$&#x20;
>
